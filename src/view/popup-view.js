...
import {generateFilm, createComments} from '../mock/film.js';
...
const createPopupTemplate = () => {
  ...
  const commentsArray = createComments(20);//все комменты
  const fragment = new DocumentFragment();
  const renderCommentList = () => {
    comments.forEach((id) => {

      const {author, comment, date: commentDate, emotion} = commentsArray[id];
